...
public class updateMyChild extends JDialog {

	private static final long serialVersionUID = 1L;
	private final JPanel contentPanel = new JPanel();
	private static myDrinkParentserviceImpl mdpi = new myDrinkParentserviceImpl();
	private static MyDrink1serviceImpl mdsi = new MyDrink1serviceImpl();
	private JTextField textField;
	private JTextField textField_1;
	private JTextField textField_2;
	myDrinkParent mm =  new myDrinkParent();
	myDrink1  mm1 =  new myDrink1 ();
	/**
	 * Launch the application.
	 */
	public static void main(String[] args) {
		try {
			updateMyChild dialog = new updateMyChild();
	 ...
	 * Create the dialog.
	 */
	public updateMyChild() {
		mm = (myDrinkParent)Cal.ReadObject("tmpfile.txt");
		mm1 = (myDrink1)Cal.ReadObject("tmpfile_1.txt");
		setTitle("產品細項修改");
				...
				okButton.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						if(textField.getText().trim().isEmpty())
							 JOptionPane.showMessageDialog(updateMyChild.this, "產品名稱不可空白", "錯誤", JOptionPane.ERROR_MESSAGE);
						else
						{
						
						  List<myDrink1> l = mdsi.selectByParentidAndDrinkName(mm.getMyDrinkParenid(),textField.getText());
						  System.out.println(l.size());
						  if (l.size()>0  && l.get(0).getDrinkid()!= mm1.getDrinkid()) {
						 	JOptionPane.showMessageDialog(updateMyChild.this, "此產生類別已有相同名稱", "錯誤", JOptionPane.ERROR_MESSAGE);
						  }
						  else
						  {
						 	mm1.setDrinkname(textField.getText());
							mm1.setMydrinkparentid(mm.getMyDrinkParenid());
							mm1.setDrinkdescription(textField_1.getText());
							mm1.setDrinkprice(Integer.valueOf(textField_2.getText()));
							mm1.setDrinkcategory("");
							mdsi.update(mm1);
							JOptionPane.showMessageDialog(updateMyChild.this, "修改資料成功");
